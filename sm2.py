...
class CryptSM2(object):
    ...
    def audit(self,data,cipher):
        #零知识证明
        m=data.hex()
        c=cipher.hex()
        a = func.random_hex(self.para_len)
        len_2 = 2 * self.para_len
        C1= c[0:len_2]
        A=self._kg(int(a,16),C1)
        h=sm3.sm3_hash([i for i in bytes.fromhex('%s%s%s%s'% (m,c,A[0:self.para_len],A[self.para_len:2*self.para_len]))])
        b=(int(a,16)+int(self.private_key,16)*int(h,16)) % int(self.ecc_table['n'],16)
        bc1=self._kg(b,C1)
        S=self._kg(int(self.private_key,16),C1)
        hS=self._kg(int(self.ecc_table['n'],16)-int(h,16),S)
        result=self._convert_jacb_to_nor(self._add_point(bc1,hS))
        return (h,b,result,A) 
