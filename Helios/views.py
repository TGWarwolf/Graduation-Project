...
def create():
    if request.method == 'POST' and 'user' in session:
        ...
        vot=Vote(name=request.json['nm'],descrip=request.json['des'],is_pri=request.json['ifp'],start=start,end=end,admin_user=session['user'],admin_pkey=pkey,complete=False,finish=False,audit=False)
        db.session.add_all([vot])
        db.session.commit()
...
def vote(id):
    ...
    question_list=[]
    if vot.questions:
        for qst in vot.questions:
            question_list.append([qst.question_text,qst.options])
    if vot.email_list:
        for em in vot.email_list:
            email_list.append(em.email_add)
    if not 'user' in session:
        return render_template("vote_guest.html",vote=vot,questions=question_list,ballot=None)
    if not session['user']==vot.admin_user:       
        ballot=Ballot.query.filter(and_(Ballot.vote_id==id, Ballot.email==session['email'])).first()
        return render_template("vote_guest.html",vote=vot,questions=question_list,ballot=ballot,user=user)
    return render_template("vote_admin.html",vote=vot,email=email_list,questions=question_list,user=user)
...
def vote_finish(id):
    ...
    if not 'user' in session:
        return render_template("vote_finish.html",vote=vot,questions=question_list,ballot=None,ballot_list=blt_list,user=user,result=result)
    if not session['user']==vot.admin_user:
        ballot=Ballot.query.filter(and_(Ballot.vote_id==id, Ballot.email==session['email'])).first()
        return render_template("vote_finish.html",vote=vot,questions=question_list,ballot=ballot,ballot_list=blt_list,user=user,result=result)
    return render_template("vote_finish.html",vote=vot,questions=question_list,ballot=None,ballot_list=blt_list,user=user,result=result)
...
def vote_audit(id):
    ...
    if not 'user' in session:
        return render_template("vote_audit.html",vote=vot,ballot=None,ballot_list=blt_list,user=user,plain_list=plain_list)
    if not session['user']==vot.admin_user:
        ballot=Ballot.query.filter(and_(Ballot.vote_id==id, Ballot.email==session['email'])).first()
        return render_template("vote_audit.html",vote=vot,ballot=ballot,ballot_list=blt_list,user=user,plain_list=plain_list)
    return render_template("vote_audit.html",vote=vot,ballot=None,ballot_list=blt_list,user=user,plain_list=plain_list)
@app.route('/delete')
def delete():
    '''
